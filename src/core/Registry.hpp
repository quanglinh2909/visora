...
// The extension mechanism, for hardware backends and business plug-ins alike.
//
// It lives in core rather than hal because "several implementations, pick the
// best one that can run here, say why the others could not" is not a property of
// hardware. Result sinks use it to let a deployment add an MQTT or webhook
// output without touching the pipeline.
//
